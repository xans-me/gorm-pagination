...
// PostgresPaginator handles pagination for PostgreSQL
...
func (p *PostgresPaginator) Paginate(db *gorm.DB, prefetchSize int) *gorm.DB {
	offset := (p.Page - 1) * p.PageSize
	return db.Offset(offset).Limit(p.PageSize + prefetchSize)
}

func (p *PostgresPaginator) ApplySorting(db *gorm.DB) *gorm.DB {
...
func (p *PostgresPaginator) PrefetchNextPage(db *gorm.DB) {
...
func (p *PostgresPaginator) GetPrefetchedData() ([]interface{}, error) {
	p.prefetchLock.Lock()
	defer p.prefetchLock.Unlock()
...
func (p *PostgresPaginator) GroupData(db *gorm.DB, groupByField string, filters map[string]interface{}, customSelect, customGroupBy, customOrder string) ([]map[string]interface{}, error) {
